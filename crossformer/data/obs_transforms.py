"""
Contains observation-level transforms used in the crossformer data pipeline. These transforms operate on the
...
def augment(obs: dict, seed: tf.Tensor, augment_kwargs: dict | Mapping[str, dict]) -> dict:
    ...
    if "augment_order" in augment_kwargs:
        augment_kwargs = dict.fromkeys(image_names, augment_kwargs)
...
def decode_and_resize(
    ...
    if isinstance(resize_size, tuple):
        resize_size = dict.fromkeys(image_names, resize_size)
    if isinstance(depth_resize_size, tuple):
        depth_resize_size = dict.fromkeys(depth_names, depth_resize_size)
