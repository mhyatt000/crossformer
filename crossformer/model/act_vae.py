...
class TransformerBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, *, deterministic: bool, attn_mask: jnp.ndarray | None):
        h = nn.LayerNorm()(x)
        h = nn.SelfAttention(
        ...
        x = x + h
        h = nn.LayerNorm()(x)
        h = nn.Dense(int(self.d_model * self.mlp_ratio))(h)
        h = nn.gelu(h)
...
class ACTVAEModel(nn.Module):
    ...
    def __call__(
        ...
        *,
        actions_chunk: jnp.ndarray | None = None,  # [B,H,8] (train ver)
        train: bool = True,
    ):
        B, *_ = images.shape
        ...
        action_queries = self.param("action_queries", nn.initializers.normal(0.02), (Hchunk, self.d_model))
