...
class Config:
    ...
    mix_version: str | None = None  # defaults to latest
...
def _resolve_version(mix_version: str | None, dataset_name: str = "xgym_sweep_single") -> str:
    """Return mix version if given, else the latest version on disk"""
...
def main(cfg: Config):
    ...
    DataSource.REGISTRY[cfg.mix].version = version
    print(f" {cfg.mix}: {version}")
